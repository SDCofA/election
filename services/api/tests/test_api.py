...
def test_health_and_catalog():
    ...
    assert status["forecast_ready"] == 0
    assert status["calendar_only"] == 19
    ...
    assert turkiye["forecast_enabled"] is False
    ...
    assert argentina["coverage_status"] == "calendar_only"
...
def test_every_public_election_has_a_catalog_jurisdiction_and_source():
    ...
    for election in elections:
        ...
        assert detail.jurisdiction.forecast_enabled is False
...
def test_public_cache_and_security_headers():
    latest = get_repository().forecasts["us-2028-president"]
...
def test_forecast_contract_and_exploratory_forecasts():
    ...
    assert detail["forecast"] is None
    assert detail["jurisdiction"]["coverage_status"] == "calendar_only"
    assert "withheld" in detail["election"]["status"].lower()
    ...
    assert turkiye.json()["forecast"] is None
    ...
    assert client.get("/v1/elections/tr-next-president/forecast").status_code == 404
    ...
    assert australia["forecast"] is None
...
def test_new_g20_country_forecasts_use_country_specific_mechanics():
    ...
    assert indonesia["forecast"] is None
...
def test_gaussian_and_markov_are_distinct_million_run_challengers():
    ...
    assert client.get(f"{base}/gaussian_monte_carlo").status_code == 404
    assert client.get(f"{base}/markov_momentum").status_code == 404


def test_immutable_snapshot_and_public_contract_surfaces():
    election_id = "us-2028-president"
    assert client.get(f"/v1/elections/{election_id}/forecast").status_code == 404
    assert client.get(f"/v1/elections/{election_id}/forecasts").status_code == 404
    assert client.get(f"/v1/elections/{election_id}/simulations").status_code == 404
    assert client.get(f"/v1/elections/{election_id}/drivers").status_code == 404
    ...
    assert client.get("/v1/elections/de-next-bundestag/coalitions").status_code == 404
